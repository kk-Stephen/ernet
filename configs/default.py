...
_C.TRANSFORMER.NUM_FEATURE_LEVELS = 5
_C.TRANSFORMER.DEC_N_POINTS = 5
_C.TRANSFORMER.ENC_N_POINTS = 5
_C.TRANSFORMER.WITH_BOX_REFINE = False
_C.TRANSFORMER.TWO_STAGE = False 

# matcher
_C.MATCHER = CN()
_C.MATCHER.COST_CLASS = 1
_C.MATCHER.COST_BBOX = 5
_C.MATCHER.COST_CIOU = 2

# LOSS
_C.LOSS = CN()
_C.LOSS.AUX_LOSS = True
_C.LOSS.IOU_AWARE = True
_C.LOSS.USE_DN = True
_C.LOSS.IOU_AWARE_COEF = 2
_C.LOSS.DICE_LOSS_COEF = 1
_C.LOSS.DET_CLS_COEF = [1, 1]
_C.LOSS.REL_CLS_COEF = 1
_C.LOSS.BBOX_LOSS_COEF = [5, 5]
_C.LOSS.CIOU_LOSS_COEF = [2, 2]
_C.LOSS.EOS_COEF = 0.1
_C.LOSS.DDF_COEF = [1,1]
...
# train
_C.TRAIN = CN()
_C.TRAIN.OPTIMIZER = 'madgrad'
_C.TRAIN.LR = 0.0001
_C.TRAIN.LR_MIN = 1e-7
_C.TRAIN.DECAY_RATE = 0.1
_C.TRAIN.LR_WARMUP = 1e-6
...
_C.TRAIN.LR_DROP = [55,110]
_C.TRAIN.CLIP_MAX_NORM = 0.1
_C.TRAIN.MAX_EPOCH = 100
_C.TRAIN.LR_SCHEDULER = 'MultiStepLR'
_C.TRAIN.WARMUP_INIT_FACTOR = 0.1
_C.TRAIN.WARMUP_STEP = 5
_C.TRAIN.WARMUP_EPOCH = 0
# Progressive Learning Epoch Steps
_C.TRAIN.PL_STAGE = [0,30,60,90,120]
# train resume

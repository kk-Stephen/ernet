...
from linformer import LinearMultiheadAttention
from XCA import XCA
...
class Linear(nn.Linear):
    ...
    def forward(self,x):
        ...
        weight = (weight - weight_mean) / std.expand_as(weight)
...
class DeformableTransformer(nn.Module):
    ...
    def __init__(self, 
        ...
        if two_stage:
            ...
            self.hs_pos_trans = nn.Linear(d_model * 2, d_model * 2)
        else:
        ...
        # ReZero
        self.enc_output_resweight = torch.nn.Parameter(torch.Tensor([1e-5]), requires_grad = True)
        self.hs_resweight = torch.nn.Parameter(torch.Tensor([1e-5]), requires_grad = True)
    ...
    def gen_encoder_output_proposals(self, memory, memory_padding_mask, spatial_shapes):
        ...
        # ReZero Memory
        output_memory2 = self.enc_output(output_memory_base)
        output_memory = output_memory_base + self.enc_output_resweight * output_memory2
    ...
    def forward(self, srcs, masks, query_embed, pos_embeds):
        ...
        if self.two_stage:
            ...
            # of Efficient DETR
            enc_outputs_class, _ = self.decoder.class_embed[self.decoder.num_layers-1](output_memory)
            enc_outputs_coord_unact = self.decoder.bbox_embed[self.decoder.num_layers-1](output_memory) + output_proposals

            ####### Hack for objectness score ########
            topk = self.two_stage_num_proposals
            hs_topk_proposals = torch.topk(torch.max(enc_outputs_class.softmax(dim=-1)[...,1:-1],dim=-1)[0], topk, dim=1)[1]
            # hs_topk_coords_unact = torch.stack([enc_outputs_coord_unact[idx] for idx in [[i,_] for i,_ in enumerate(hs_topk_proposals)]],dim=0)
            hs_topk_coords_unact = torch.gather(enc_outputs_coord_unact, 1, hs_topk_proposals.unsqueeze(-1).repeat(1, 1, 4))
            hs_topk_coords_unact = hs_topk_coords_unact
            hs_reference_points = hs_topk_coords_unact.sigmoid()
            hs_init_reference_out = hs_reference_points
            
            # ReZero Instance reference points
            hs_topk_coords_unact_proposals = self.get_proposal_pos_embed(hs_topk_coords_unact)
            hs_pos_trans_out = self.hs_pos_trans(hs_topk_coords_unact_proposals)
            hs_pos_trans_out = hs_topk_coords_unact_proposals + self.hs_resweight * hs_pos_trans_out        

            # Instance Encoder features as obj queries
            _ , tgt = torch.split(hs_pos_trans_out, c, dim=2)
            query_embed = torch.stack([output_memory[idx] for idx in [[i,_] for i,_ in enumerate(hs_topk_proposals)]],dim=0)
        ...
        # decoder
        hs, hs_inter_references = self.decoder(tgt, hs_reference_points, 
...
class DeformableTransformerEncoderLayer(nn.Module):
    def __init__(self,
        ...
        self.resweight = torch.nn.Parameter(torch.Tensor([1e-5]), requires_grad = True)
...
class DeformableTransformerDecoderLayer(nn.Module):
    def __init__(self, d_model=256, d_ffn=1024,
        ...
        # self.self_attn = LinearMultiheadAttention(d_model, n_heads, dropout=dropout, seq_len=seq_len, proj_k=64)
        # self.self_attn = XCA(d_model, n_heads, attn_drop=dropout)        
    ...
    def forward_ffn(self, tgt):
        ...
        tgt = tgt + self.dropout4(tgt2) * self.resweight
...
def build_deformable_transformer(cfg):
    if cfg.TRANSFORMER.BRANCH_AGGREGATION is False:
        ...
        return DeformableTransformer(
            ...
            dropout=cfg.TRANSFORMER.DROPOUT[0],
